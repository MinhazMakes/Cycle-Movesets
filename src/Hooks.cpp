...
void ParsePerkListJsonFor2H(const rapidjson::Value& sourceObject, const std::string& keyName,
    ...
    for (const auto& perkData : perkArray) {
        if (perkData.IsArray() && perkData.Size() == 3 && perkData[0].IsString() && perkData[1].IsString() && perkData[2].IsBool()) {
            std::string plugin = perkData[0].GetString();
            std::string hexID = perkData[1].GetString();
            RE::FormID formID = ResolveFormID(plugin, hexID);
            bool isDisabled = perkData[2].GetBool();
            if (formID != 0) {
                targetList.push_back({ plugin, formID, "", isDisabled });
            }
        }
    }
}
...
void CreateHitRulesJson(rapidjson::Document& doc, rapidjson::Value& targetObject, const std::string& keyName,
    ...
    for (const auto& rule : hitRulesList) {
        ...
        CreatePerkListJsonFor2H(doc, ruleObj, "DisabledPerks", rule.disabledPerks);
...
void ParseHitRulesJson(const rapidjson::Value& sourceObject, const std::string& keyName,
    ...
    for (const auto& ruleObj : rulesArray) {
        if (ruleObj.IsObject() && ruleObj.HasMember("HitCount") && ruleObj["HitCount"].IsInt()) {
            ...
            ParsePerkListJsonFor2H(ruleObj, "DisabledPerks", newRule.disabledPerks);
...
void CreateStartupRulesJson(rapidjson::Document& doc, rapidjson::Value& targetObject, const std::string& keyName,
    const std::vector<StartupRule>& rulesList) {
    if (rulesList.empty()) return;
    ...
    for (const auto& rule : rulesList) {
        rapidjson::Value ruleObj(rapidjson::kObjectType);
        ruleObj.AddMember("Name", rapidjson::Value(rule.name.c_str(), allocator), allocator);

        // Reutiliza os helpers existentes para salvar as listas de perks e efeitos
        CreatePerkListJsonFor2H(doc, ruleObj, "RequiredPerks", rule.requiredPerks);
        CreatePerkListJsonFor2H(doc, ruleObj, "DisabledPerks", rule.disabledPerks);
        CreateEffectListJson(doc, ruleObj, "AppliedEffects", rule.effects);
...
void ParseStartupRulesJson(const rapidjson::Value& sourceObject, const std::string& keyName,
    std::vector<StartupRule>& targetList) {
    targetList.clear();
    if (!sourceObject.HasMember(keyName.c_str()) || !sourceObject[keyName.c_str()].IsArray()) {
        return;
    }
    ...
    const auto& rulesArray = sourceObject[keyName.c_str()].GetArray();
    for (const auto& ruleObj : rulesArray) {
        if (ruleObj.IsObject() && ruleObj.HasMember("Name")) {
            StartupRule newRule;
            newRule.name = ruleObj["Name"].GetString();

            // Reutiliza os helpers de parse
            ParsePerkListJsonFor2H(ruleObj, "RequiredPerks", newRule.requiredPerks);
            ParsePerkListJsonFor2H(ruleObj, "DisabledPerks", newRule.disabledPerks);
            ParseEffectListJson(ruleObj, "AppliedEffects", newRule.effects);
                ...
                for (int i = 0; i < stanceLimit; ++i) {  // Stances
                    ...
                    for (size_t mod_idx = 0; mod_idx < instance.modInstances.size(); ++mod_idx) {
                        ...
                        for (const auto& subInst : modInst.subAnimationInstances) {
                            ...
                            CreateStartupRulesJson(doc, animObj, "StanceStartupRules", instance.startupRules);
                            CreateStartupRulesJson(doc, animObj, "MovesetStartupRules", modInst.startupRules);
                            CreateStartupRulesJson(doc, animObj, "SubMovesetStartupRules", subInst.startupRules);
                ...
                for (const auto& categoryJson : menu.GetArray()) {
                    ...
                    for (const auto& stanceJson : categoryJson["stances"].GetArray()) {
                        ...
                        for (const auto& animJson : stanceJson["animations"].GetArray()) {
                            ...
                            targetInstance.startupRules.clear();
                            modInstancePtr->startupRules.clear();
                            newSubInstance.startupRules.clear();
                            ...
                            ParseStartupRulesJson(animJson, "StanceStartupRules", targetInstance.startupRules);
                            ParseStartupRulesJson(animJson, "MovesetStartupRules", modInstancePtr->startupRules);
                            ParseStartupRulesJson(animJson, "SubMovesetStartupRules", newSubInstance.startupRules);
...
void AnimationManager::LoadAnimationLibrary() {
    ...
    if (!Settings::DisableCache && ValidateCache(cachePath)) {
...
void AnimationManager::DrawConditionsEffectsPopup() {
    if (_isConditionsEffectsPopupOpen && !_hitRuleToEdit && !_startupRuleToEdit) {
        if (_stanceToEditPerk || _stanceToEditEffect) {
            ...
            _startupRuleListOwner = _stanceToEditPerk ? &_stanceToEditPerk->startupRules : &_stanceToEditEffect->startupRules;
        }
            ...
            _startupRuleListOwner = _movesetToEditPerk ? &_movesetToEditPerk->startupRules : &_movesetToEditEffect->startupRules;
        }
        else if (_subMovesetToEditPerk || _subMovesetToEditEffect) {
            _hitRuleListOwner = _subMovesetToEditPerk ? &_subMovesetToEditPerk->hitRules : &_subMovesetToEditEffect->hitRules;
            _startupRuleListOwner = _subMovesetToEditPerk ? &_subMovesetToEditPerk->startupRules : &_subMovesetToEditEffect->startupRules;
        }
    }
    ...
    std::vector<StartupRule>* targetRules = nullptr;
    ...
    bool modalOpen = _isConditionsEffectsPopupOpen;
    if (ImGui::BeginPopupModal("Conditions & Effects", &_isConditionsEffectsPopupOpen, ImGuiWindowFlags_None)) {
        if (!modalOpen) {
            if (_hitRuleToEdit || _startupRuleToEdit) {
                // Se estava editando algo, apenas volta, não fecha tudo
                _hitRuleToEdit = nullptr;
                _startupRuleToEdit = nullptr;
        ...
        bool isEditingStartupRule = (_startupRuleToEdit != nullptr);
        ...
        if (ImGui::BeginTabBar("ConditionsEffectsTabs")) {
                ...
                if (ImGui::BeginChild("PerkListChild", ImVec2(0, -ImGui::GetFrameHeightWithSpacing() * 2))) {
                    ...
                    if (perkDisplayFilter == 0) {  // "All"
                        for (const auto& perk : _allPerks) {
                            ...
                            std::string perk_editorid_lower = perk.editorID;
                            std::transform(perk_name_lower.begin(), perk_name_lower.end(), perk_name_lower.begin(),
                                ::tolower);
                            std::transform(perk_editorid_lower.begin(), perk_editorid_lower.end(), perk_editorid_lower.begin(), ::tolower);

                            if (filter_str.empty() ||
                                perk_name_lower.find(filter_str) != std::string::npos ||
                                perk_editorid_lower.find(filter_str) != std::string::npos) {
                        ...
                        for (auto it = tempSelectedPerks.begin(); it != tempSelectedPerks.end(); /* no increment */) {
                            ...
                            std::string perk_name_lower = perk->name;
                            std::string perk_editorid_lower = perk->editorID; // Adicionado

                            std::transform(perk_name_lower.begin(), perk_name_lower.end(), perk_name_lower.begin(), ::tolower);
                            std::transform(perk_editorid_lower.begin(), perk_editorid_lower.end(), perk_editorid_lower.begin(), ::tolower); // Adicionado
                            ...
                            if (filter_str.empty() ||
                                ...
                                if (is_selected && !is_inherited) {
                                    ...
                                    // Como estamos iterando 'tempSelectedPerks', 'it' já aponta para o dado correto
                                    // mas precisamos garantir que estamos alterando a flag correta
                                    int typeIndex = selectedPerkDef.isDisabled ? 1 : 0;
                                    const char* types[] = { "Required", "Disabler" };
                                    std::string comboID = std::format("##TypeSel_{:X}", selectedPerkDef.formID);

                                    if (ImGui::Combo(comboID.c_str(), &typeIndex, types, 2)) {
                                        // Atualiza o valor no iterador (que aponta para tempSelectedPerks)
                                        it->isDisabled = (typeIndex == 1);
                                    }
                                    ImGui::PopItemWidth();
                                }
                                if (is_inherited) ImGui::EndDisabled();
            ...
            if (isEditingHitRule || isEditingStartupRule) {
                // --- Aba de Apply Effects ---
                if (ImGui::BeginTabItem("Apply Effects")) {
            ...
            if (!isEditing2HConfig && !isEditingHitRule && !isEditingStartupRule) {  // Não mostra esta aba se estiver editando 2H ou uma Hit Rule
                if (ImGui::BeginTabItem("Startup Rules")) {
                    if (ImGui::Button("Add New Startup Rule")) {
                        _isConditionsEffectsPopupOpen = false;
                        _isStartupRuleNamePopupOpen = true;
                        strcpy_s(_startupRuleNameBuffer, "");
                        _startupRuleToEdit = nullptr;
                    }
                    ImGui::Separator();

                    if (ImGui::BeginTable("StartupRulesTable", 3, ImGuiTableFlags_Borders | ImGuiTableFlags_RowBg)) {
                        ImGui::TableSetupColumn("Rule Name", ImGuiTableColumnFlags_WidthStretch);
                        ImGui::TableSetupColumn("Details", ImGuiTableColumnFlags_WidthStretch);
                        ImGui::TableSetupColumn("Actions", ImGuiTableColumnFlags_WidthFixed, 150.0f);
                        ImGui::TableHeadersRow();

                        if (_startupRuleListOwner) {
                            for (auto it = _startupRuleListOwner->begin(); it != _startupRuleListOwner->end();) {
                                ImGui::PushID(&(*it));
                                ImGui::TableNextRow();
                                ImGui::TableNextColumn();
                                ...
                                ImGui::TableNextColumn();
                                ImGui::Text("Perks: %zu | Effects: %zu", it->requiredPerks.size(), it->effects.size());

                                ImGui::TableNextColumn();
                                if (ImGui::Button("Edit")) {
                                    _startupRuleToEdit = &(*it);
                                    tempSelectedPerks = it->requiredPerks;
                                    // Adiciona perks desabilitados à lista temporária
                                    for (auto& dp : it->disabledPerks) tempSelectedPerks.push_back(dp);
                                    tempSelectedEffects = it->effects;

                                    _inheritedPerkFormIDs.clear();
                                    _inheritedEffectFormIDs.clear();
                                }
                                ImGui::SameLine();
                                if (ImGui::Button("Delete")) it = _startupRuleListOwner->erase(it);
                                else ++it;
                        ...
                        if (_hitRuleListOwner) {
                            for (auto it = _hitRuleListOwner->begin(); it != _hitRuleListOwner->end();) {
                                ...
                                if (ImGui::Button("Edit")) {
                                    ...
                                    for (auto p : rule.disabledPerks) {
                                        p.isDisabled = true;
                                        _perksToDisplayInPopup.push_back(p);
                                    }
        ...
        if (ImGui::Button(LOC("save"), ImVec2(120, 0))) {
            ...
            if (_hitRuleToEdit) {
                ...
                _hitRuleToEdit->effects = effectsToSave;
                _hitRuleToEdit = nullptr; // Retorna ao menu anterior
                tempSelectedPerks.clear(); // Limpa para não poluir a tela principal
                tempSelectedEffects.clear();
            }
            else if (_startupRuleToEdit) {
                _startupRuleToEdit->requiredPerks = perksToSave;
                _startupRuleToEdit->disabledPerks = disabledPerksToSave;
                _startupRuleToEdit->effects = tempSelectedEffects;
                _startupRuleToEdit = nullptr; // Retorna ao menu anterior
                tempSelectedPerks.clear();
                tempSelectedEffects.clear();
            }
            else if (_editingPlayer2HPerks) {
...
void AnimationManager::DrawStartupRuleNamePopup() {
    if (_isStartupRuleNamePopupOpen) {
        ImGui::OpenPopup("New Startup Rule");
    }
    ...
    if (ImGui::BeginPopupModal("New Startup Rule", &_isStartupRuleNamePopupOpen, ImGuiWindowFlags_AlwaysAutoResize)) {
        ImGui::Text("Enter a name for the startup rule:");
        ImGui::InputText("##SRName", _startupRuleNameBuffer, sizeof(_startupRuleNameBuffer));

        if (ImGui::Button("Create", ImVec2(120, 0))) {
            if (strlen(_startupRuleNameBuffer) > 0 && _startupRuleListOwner) {
                StartupRule newRule;
                newRule.name = _startupRuleNameBuffer;
                _startupRuleListOwner->push_back(newRule);

                // Entra direto na edição da nova regra
                _startupRuleToEdit = &_startupRuleListOwner->back();
                _perksToDisplayInPopup.clear();
                _effectsToDisplayInPopup.clear();
                _inheritedPerkFormIDs.clear();
                _inheritedEffectFormIDs.clear();
                _isStartupRuleNamePopupOpen = false;
                _isConditionsEffectsPopupOpen = true;
                ImGui::CloseCurrentPopup();
            }
        }
    ...
    if (ImGui::BeginPopupModal("Enter Hit Count", &_isHitCountNumberPopupOpen, ImGuiWindowFlags_AlwaysAutoResize)) {
        ...
        if (ImGui::Button("Save", ImVec2(120, 0))) {
            if (_hitCountRuleEditorHitNumber <= 0) {
                ...
                _hitRuleListOwner->push_back(newRule);
                _hitRuleToEdit = &(_hitRuleListOwner->back());
                // Mantém a lista ordenada
                std::sort(_hitRuleListOwner->begin(), _hitRuleListOwner->end());
